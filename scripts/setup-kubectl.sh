...
HAPROXY1_PUB_IP=$(priv_ip=$(dcos task k8s-api-proxies_k8s-1 | grep -v HOST | awk '{print $2}') && dcos node ssh --option StrictHostKeyChecking=no --option LogLevel=quiet --master-proxy --private-ip=$priv_ip --user=centos "curl http://169.254.169.254/latest/meta-data/public-ipv4"); echo && echo "HAPROXY1_PUB_IP:   $HAPROXY1_PUB_IP"

HAPROXY2_PUB_IP=$(priv_ip=$(dcos task k8s-api-proxies_k8s-2 | grep -v HOST | awk '{print $2}') && dcos node ssh --option StrictHostKeyChecking=no --option LogLevel=quiet --master-proxy --private-ip=$priv_ip --user=centos "curl http://169.254.169.254/latest/meta-data/public-ipv4"); echo && echo "HAPROXY2_PUB_IP:   $HAPROXY2_PUB_IP"
